...
bool cmp(const TASK &a, const TASK &b){
        if (a.event.task.priority == Event::Task::Priority::kHigh && b.event.task.priority == Event::Task::Priority::kLow)
                return true;
        else if (a.event.task.priority == b.event.task.priority && (a.event.task.deadline-a.event.task.arrivalTime < b.event.task.deadline-b.event.task.arrivalTime))
                return true;
        else return false;
}
...
double cal(TASK T){ //计算cpu的优先级得分
		double score = 0;
		score += 5.0*(1-1.0*T.done/donemax);
		long long lefttime = T.event.task.deadline-curtime;
		lefttime = lefttime>0?lefttime:100000000; //剩余时间 对于过期任务 将其优先级放到最后
...
Action schedule() { //决定调度
        ...
        std::vector<TASK> candio,candcpu;
        for(int i = 9; i >= 0; i--) { //选择所有高等级的cpu任务
            donemax = 1;
            for(auto iter = list.begin(); iter != list.end(); iter++) {
                if(iter->event.type == Event::Type::kTaskArrival && iter->level == i) {
                    if(iter->done > donemax)
                        donemax = iter->done;
                    candcpu.push_back(*iter);
                }
            }
            if(!candcpu.empty())
                break;
        ...
        if(!candcpu.empty()) {
//         for(auto iter = candcpu.begin(); iter != candcpu.end(); iter++) { //评分机制选取得分最高的cpu
//             double sco = cal(*iter);
//             if(sco > maxcpu) {
//                 maxcpu = sco;
...
//             }
//         }
//--------简单排序机制,注释此段,取消上面评分机制注释,为评分机制选取,由于参数问题只拿了最多79分---------------
			sort(candcpu.begin(), candcpu.end(), cmp); //对高优先级cpu任务进行排序
...
//----------------------------------------------------------------------------------------------
        if(io_id) //io 不空闲
			return ans;
        for(int i = 9; i >= 0; i--) { //选择所有高等级的io任务
            donemax = 1;
            for(auto iter = list.begin(); iter != list.end(); iter++) {
                if(iter->event.type == Event::Type::kIoRequest && iter->level == i) {
                    if(iter->done > donemax)
                        donemax = iter->done;
                    candio.push_back(*iter);
                }
            }
            if(!candio.empty())
                break;
        ...
        if(!candio.empty()) {
//         for(auto iter = candio.begin(); iter != candio.end(); iter++) { //评分机制选取得分最高的io
//             double sco = cal(*iter);
//             if(sco > maxio) {
//                 maxio = sco;
...
//         }
//--------简单排序机制,注释此段,取消上面评分机制注释,为评分机制选取,由于参数问题只拿了最多79分---------------
			sort(candio.begin(), candio.end(), cmp); //对最高优先级io任务进行排序
